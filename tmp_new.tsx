...
export default function NewJobPage() {
  ...
  // File Upload State
  const [files, setFiles] = useState<{file: File, id: string | null}[]>([]);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const newFileObjs = validFiles.map(f => ({ file: f, id: null }));
      ...
      for (const obj of newFileObjs) {
        try {
          ...
          setError(`PDFのアップロードに失敗しました(${obj.file.name}): ` + err.message);
        }
      }
      setUploading(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'jobPostings'), {
        ...
        pdfFileIds: files.filter(f => f.id).map(f => f.id),
        pdfFileId: files.find(f => f.id)?.id || null, // legacy
                      ...
                      <div key={idx} className="flex items-center justify-between bg-white p-3 rounded-lg border">
                         <div className="flex items-center gap-3">
                            ...
                            <div className="text-sm">
                               <p className="font-medium truncate max-w-[200px]">{fileObj.file.name}</p>
